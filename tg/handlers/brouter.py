...
async def send_photo_to_op(msg: Message, bot: Bot):
    ...
    if last_usage:
        ...
        builder.add(InlineKeyboardButton(text=f"✅ ({last_usage.usage_inv.amount_in_fiat} {last_usage.usage_req.country.country}) {short_name} *{last_digits}", callback_data=f"accept_invoice_{last_usage.usage_inv.id}"))
